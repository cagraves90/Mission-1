...
// expect(sum.carValue(-2000)).toBe("Invalid input");
// describe("400 error when car in API 1 is incorrect data type", () => {
//   test("should response with a status code of 404", async () => {
//     const bodyData = [{ year: "Twenty twenty" }];
//     for (const body of bodyData) {
//       const response = await request(app)
//         .get("/carModelYearRequest")
//         .send(body);
//       expect(response.statusCode).toBe(404);
//     }
//   });

//   test("should return 400 if model is not a string", async () => {
//     const response = await request(app)
//       .get("/carModelYearRequest")
//       .send({ model: null, year: 2020 });
...
//   test("should return 400 if carYear is blank", async () => {
//     const bodyData = [{ carYear: null }];
//     for (const body of bodyData) {
//       const response = await request(app)
//         .post("/carModelYearRequest")
//         .send(body);
//       expect(response.statusCode).toBe(400);
//     }
//   });

//   test("should return 400 if carName is blank", async () => {
//     const bodyData = [{ carName: null }];
//     for (const body of bodyData) {
//       const response = await request(app).post("/carInfo").send(body);
//       expect(response.statusCode).toBe(400);
//     }
//   });
// });

// describe("if API has input of incorrect data type, should return 400", () => {

// test("should return 200 if carYear is provided with number", async () => {
//   // const response = await request(app).post("/carInfo").query({ carYear: 2023 });
//   const bodyData = [{ carYear: 2000 }];
//   for (const body of bodyData) {
//     const response = await request(app).post("/carInfo").send(body);
...
//--------------------------------------------------------- API endpoint using Jest and Supertest DEMO ---------------------------------------- ///
